...
app.post('/api/pacientes/login', async (req, res) => {
    ...
    try {
        ...
        // RETORNA O CPF para o Frontend armazenar (necessário para a segurança do agendamento)
...
// CORREÇÃO CRÍTICA: ROTA AGENDAR CONSULTA (Método POST)
...
app.post('/api/agendamentos', protegerRota, async(req, res) =>{
    ...
    try{
        ...
        if (req.userTipo === 'paciente') {
            ...
            // O frontend já impede isso, mas a validação no backend é essencial.
            ...
            if (pacientes.length === 0 || pacientes[0].CPF !== CPFPaciente) {
                 return res.status(403).json({ success: false, message: 'Acesso negado. Você só pode agendar para seu próprio CPF.' });
            }
        }
        
        // Validação de campos
        if (!ID_Medico || !CPFPaciente || !DataHora || !EspecialidadeDesejada) {
            return res.status(400).json({ success: false, message: 'Todos os campos de agendamento (ID_Medico, CPF, Data/Hora, Especialidade) são obrigatórios.' });
        }
        ...
        if(pacientes.length === 0){
            // Se o paciente não existir, podemos retornar um 404
        ...
        // A DataHora já deve estar no formato ISO8601 (YYYY-MM-DDTHH:MM:00) vindo do frontend, 
        // o que o MySQL/mysql2 geralmente aceita
        
        // 2. Configurações de agendamento padrão
